...
and a head's length out as well; the entry in the slot vocabulary below has the
numbers.
...
- **`.elbow`'s half of `edge-short`.** `dgEdgeShortWarnings` measures an elbow on its **arrival run** rather than on the whole exposed route, because an elbow's exposed route is mostly rail and the rail says nothing. It therefore fires exactly where the span is under `DG_ELBOW_ARRIVE + DG_ELBOW_LEAVE` (36.75 px), names the run in px and in labels and names the two fixes – more gap, or `via`. The **headless exemption is the same one the straight run keeps** and it matters here: a tree written `--` has no head to crowd, and `docs/artifact/figure-rules` `#tree` is six of them at spans the floor would otherwise report.
