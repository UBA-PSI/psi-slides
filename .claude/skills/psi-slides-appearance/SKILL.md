...
`STYLE_SPEC` in build.js is the whole `style:` block, mirrored in `lint.js` as `STYLE_ENUMS` (the enums only – the two scales are bounded numbers, and reading a number out of YAML with no parser is where a linter starts disagreeing with the build). The keys: `headings` (auto/left/center/off), `rules` (on/off), `labels` (on/off), `link-codes` (on/off), `wrap` (balance/none), `blocks` (center/left), `hyphenate` (print/all/none), `print-body` (serif/sans), `bold` and `print-bold` (plain/bold/italic/accent/accent-bold/accent-italic), `reveal` (grow/hold – what a top-level `---` segment does before its beat: `grow` closes it up and the chunk grows per press, the 1.0.0 behaviour and the default; `hold` lays the chunk out at its final height from beat 0 and the segment fades in where it was going to be, which is what a beat below the top level does anyway – `body[data-reveal=hold]`, and the speaker's ghost is then the box at half strength in the flow), `heading-scale` and `body-scale` (0.6–1.8).
