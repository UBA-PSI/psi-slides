...
**`reader` is the one key here that no live view reads.** It is about the two documents, `print.html` and `print-notes.html`, as a reader has them on a screen after the lecture: under `on` (the default) they carry a contents sidebar – one entry per slide, grouped by part, numbered with the number the document prints, the title chunk and an `outline:` chunk left out – that marks the slide the reader is in (`aria-current="location"`, the entry whose top last crossed 30% of the window), and the layout keeps a 17rem column free on the right for the reader's own notes. `off` ships none of it: no sidebar, no highlights, no reader script (`PRINT_READER_JS`, `PRINT_HIGHLIGHTS_JS`, `READER_EARLY_JS`), no `reader-data` block, no `data-reader` on the body. The lightbox stays either way, because it is how a figure is read rather than a tool of the reader's. The rules in `PRINT_CSS` are keyed off `body[data-reader=on].rd-ready`, and the class is set by a one-line script right after the sidebar, so a page read without scripts is laid out as before and a paper print never sees any of it.

The layout has three widths, solved rather than guessed, because the screen root size follows the window (`clamp(15px, calc(12px + 0.3vw), 18px)`) and a media query cannot read it. From `READER_WIDE_PX` (1216) the sidebar stands fixed on the left – 15rem of contents, the 42rem text, 17rem of notes and the air between them come to 77rem, 1201 px at that width. From `READER_NOTES_PX` (920) up to there it folds to a button top left that opens it over the page, and the notes column still fits. Below 920 there is no notes column and the text is centred as it always was. Both numbers are constants interpolated into the stylesheet and the script, so the two cannot disagree. The sidebar's words come from `STRINGS` (`contents`, `reader-close`), so `labels:` reaches them.

**Under `on` the reader can also highlight** (`PRINT_HIGHLIGHTS_JS`, PLAN-reader-highlights.md §2–4): select words in a slide, press the button at the end of the selection, and they turn yellow with a card for an optional note – in the notes column at the height of the highlight from 920 px, inline under its paragraph and only while opened below that. A highlight is anchored to its chunk's frozen `{#id}` and to offsets into the chunk's *reader text*, which leaves out the build's own labels and numbers, speaker notes, figures, formulas and code, so the offsets are the same in both documents; a rebuild that moved the words re-anchors by the quote and its 32 characters of context, and one that removed them lists the highlight in the sidebar's foot (`[data-reader-slot=tools]`) and keeps it. The store is `localStorage` under `psi-reader:v1:<source folder>` – the build writes the folder's name into a `reader-data` JSON block with its `reader-*` words (thirteen since the navigation), so `labels:` reaches those too – and a browser that refuses storage gets session-only highlights and a notice in the foot. Once there is a highlight, a pill in the bottom right corner (`.rd-nav`) steps through them in page order with `‹ n / N ›`, and `n` / `p` do the same. It stops at the ends and can be filtered to *all* or *with note*. The contents entries show a per-slide count (`.rd-count`), and five more words carry it (`reader-nav`, `reader-prev`, `reader-next`, `reader-filter-all`, `reader-filter-notes`). Chrome and Safari share one `file://` store between the two documents; Firefox keeps one per file, so there they share only under `--serve`. The sidebar's parts fold to their headings (`.rd-group.is-open`, a `.rd-fold` chevron each, `reader-fold`): the part being read opens and closes by itself, one opened by hand stays open, and a folded part shows the sum of its counts (`.rd-sum`). The sidebar's foot holds a menu (`.rd-menu`): *export* downloads `<source folder>-<reader-export-file>.md`, readable Markdown grouped by slide with one `<!-- psi-reader {…} -->` comment per entry; *import* reads those comments only, merges by id (the later `edited` wins) and reports in one line; *delete all* has the five-second undo; a small `?` (`.rd-help-btn`) opens a few lines on what can be highlighted, the keys, where highlights are kept and why the export is the backup (`reader-help`, `reader-help-what`, `reader-help-keys`, `reader-help-where`, `reader-help-risk`, `reader-help-backup`). The key hints – the `?`'s keys line and `m` on the lightbox's spot button; the pill's arrows carry theirs in `title` and `aria-keyshortcuts` only – show only under `(hover: hover) and (pointer: fine)`, and under `(pointer: coarse)` every reader control is at least 44 by 44 CSS px. Fifteen more words carry it (`reader-export` to `reader-deleted-all`); `reader-export-line`, `reader-imported` and `reader-import-skipped` hold `{name}` slots the page fills, each count after a colon so no word has to agree with it. Highlights print (`@media print`, `print-color-adjust: exact`): yellow, and a note as a numbered float in the outer margin, rebuilt on every layout rather than on `beforeprint` because a scripted PDF fires no print event. A figure, a code block and a display formula carry a corner button that marks them whole, a code block's words take a text highlight anchored in the block's own text, and the lightbox sets a pin on a spot of a figure, snapped to a `::: draw` part by its author-given name. PLAN-reader-highlights.md §6 and §11 hold the decisions and the measurements.
