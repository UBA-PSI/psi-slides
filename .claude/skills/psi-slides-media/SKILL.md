...
The one thing on the projection that is not in `source.md` at all. `D` captures a window or a screen of the machine with `getDisplayMedia` and shows it full-frame in the audience window, letterboxed on black, until `D` again. It exists for the extended desktop: the demo runs on the laptop screen, where the pointer and the keyboard are, and the room sees it without the displays being mirrored and un-mirrored around it – which is what used to lose every window position on the way back.

Four things decide the shape, and each was measured rather than assumed:

- **The cockpit captures, the projection shows.** `getDisplayMedia` needs a user gesture in the calling window and opens its picker there. A key in the audience window would put the picker on the wall and need focus on the projector first. So the speaker's `D` captures (picker on the laptop) and hands the stream over; running alone, the audience captures and shows for itself. The cockpit never shows the picture – the lecturer is looking at the real window – it shows a badge, the blank badge's twin.
- **Two transports, chosen by origin.** `DEMO_DIRECT` is `SELF_ORIGIN !== 'null'`, known at boot: two `file://` windows both report the origin `null`, anything served reports a real one, and the two windows are one origin by construction (relative `window.open`). Under `--serve` the speaker calls `peer.psiDemoAttach(stream)` and the audience sets a cross-realm `MediaStream` as `srcObject`, no copy, no encoder, no added latency; a same-origin peer without the hook is a projection built before the demo existed, and that is said in a toast rather than silently re-routed. From `file://` the stream goes through an `RTCPeerConnection` on loopback, offer and answer and ICE carried by `demo-offer` / `demo-answer` / `demo-ice` over the existing `postMessage` link. A candidate that arrives before the remote description is in waits on `demoRemoteReady`, the promise each connection carries; every `await` in the handshake checks the connection is still the current one, so a quick stop and start leaves no older handshake speaking. The connection has no ICE servers, deliberately – a STUN server would make every `D` reach the internet – and a `failed` state ends the demo with a toast instead of leaving the room on black. Chromium 141 cannot transfer a `MediaStreamTrack` between windows in either case (`postMessage(track, [track])` refuses the type), which is why the direct path is a call and not a transfer. Because the print tab is same-origin too under `--serve`, `P` opens it with `noopener`.
- **Text through the encoder, and no more pixels than the wall has.** The capture is capped at `DEMO_MAX_W` × `DEMO_MAX_H` (2560 × 1600) and 30 fps – a Retina or 5K screen at its own size is a copy per frame nobody sees, and on loopback a size the software encoder cannot keep up with. The loopback path sets `contentHint = 'detail'` on the track, raises `maxBitrate` to `DEMO_MAX_BITRATE` (40 Mbit/s) and asks for `maintain-resolution`, because Chrome's default cap is tuned for the internet and blurs a terminal the moment it scrolls; `test/demo.mjs` reads the sender back after negotiation, so the setting is measured rather than assumed to survive it. The picker's constraints exclude this browser window (`selfBrowserSurface`) and include screens and surface switching, so Chrome's share bar can swap the source mid-demo.
- **A command, not state – and handed over again on `hello`.** Like blank: outside the snapshot, past the freeze gate, and `D` in either window ends it on both, in overview too (the stop half is the urgent one, so there is no overview guard, as on `B`). `D` while the picker is still open is ignored (`demoPending`), and a cockpit with no projection window refuses before the picker rather than capturing into nothing. `track.stop()` fires no `ended` on the far side, so the stop is always also a `demo {action: 'stop'}` message; Chrome's own "stop sharing" bar ends the track locally and takes the same path; `pagehide` on the capturing window sends the message and nothing else. A stream cannot be re-applied from a snapshot, only its holder can hand it over again: the projection sends `hello` whenever it adopts a peer, and a window holding a capture answers any `hello` with `demoAnnounce` – a reloaded projection gets the picture back on the cockpit's next push, and a cockpit opened with `S` under a demo the projection started alone learns of it. Blank hides `#demo-overlay` on the projection while the capture runs on; the overlay sits at `z-index` 32, above the figure overlay and below the touch rail, the laser pointer, the mode toast and the search panel, and `body.demo-live` stops the covered stage from painting.

`test/demo.mjs` drives both transports with a canvas stream in place of the capture, and the cases a review found: D twice with the picker open, a cockpit without a projection, D in overview, a projection reloading under a demo, and the stop from the projection side. What it cannot cover is the picker itself: the real `getDisplayMedia` call and macOS's one-time screen-recording prompt could not be exercised here – Xvfb Chromium has no desktop capturer – and want a look on a Mac with a projector. `getDisplayMedia` and WebRTC exist in Firefox and Safari too, but nothing here has been tried there.
