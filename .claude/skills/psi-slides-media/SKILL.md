...
QR codes for a lecture's links are generated **at build time** by `qrSvg()` and shipped as a `LINK_QR` map keyed by URL, so a lecture without links pays nothing for them. The one address a build cannot know is the one typed into a live annotation mid-talk, so both live views also carry the encoder itself – `qrLibJs()` reads `qrcode-generator/dist/qrcode.js` out of `node_modules` as text and splices it in as its own `<script>`, the way `diagram-core.mjs` ships, leaving a global `qrcode`; `liveQrSvg()` in `AUDIENCE_JS` calls it with the same level (M) and quiet zone (4) as the build. 56 KB per live view; print pays nothing. The white card under a code is `.qr-card`, shared by the link overlay and the annotation. Three things worth keeping:
...
- **The byte function is UTF-8 via `TextEncoder`, set on every side** (build, live views, `test/annotation.mjs`). The library's default masks each code unit to one byte, so an address with a non-Latin-1 character in its path scanned to a different string while looking right on the slide; and its ESM build has no UTF-8 table of its own, which is why it is `TextEncoder` and not the library's function. A live address the largest symbol cannot hold (the library throws past about 2.3 KB at level M) draws no code rather than throwing out of the input handler.
