...
- **Two transports, chosen at run time.** Under `--serve` both windows are one origin and the speaker calls `peer.psiDemoAttach(stream)`: the audience sets a cross-realm `MediaStream` as `srcObject`, no copy, no encoder, no added latency. From `file://` the two windows are opaque origins, the call throws `SecurityError`, and the stream goes through an `RTCPeerConnection` on loopback, offer and answer and ICE carried by `demo-offer` / `demo-answer` / `demo-ice` over the existing `postMessage` link. Candidates that arrive before the remote description is in wait in `demoIceQueue`. Chromium 141 cannot transfer a `MediaStreamTrack` between windows in either case (`postMessage(track, [track])` refuses the type), which is why the direct path is a call and not a transfer.
- **Text through the encoder.** The loopback path sets `contentHint = 'detail'` on the track, raises `maxBitrate` to `DEMO_MAX_BITRATE` (40 Mbit/s – loopback has it) and asks for `maintain-resolution`, because Chrome's default cap is tuned for the internet and blurs a terminal the moment it scrolls. The picker's constraints exclude this browser window (`selfBrowserSurface`) and include screens and surface switching, so Chrome's share bar can swap the source mid-demo.
- **A command, not state.** Like blank: outside the snapshot, past the freeze gate, and `D` in either window ends it on both. `track.stop()` fires no `ended` on the far side, so the stop is always also a `demo {action: 'stop'}` message; Chrome's own "stop sharing" bar ends the track locally and takes the same path; `pagehide` on the capturing window does too, or the projection would keep a frozen last frame. Blank hides `#demo-overlay` on the projection while the capture runs on. An audience reload during a demo loses the picture (nothing about it is in the snapshot) – `D` twice restarts it.

What is verified and what is not: both transports were driven in headless Chromium with a canvas stream in place of the capture (direct path under http, `connected` peer connection under `file://`, stop from the audience side reaching the cockpit), and the message handling and badge rules with it. The real `getDisplayMedia` call, the picker and macOS's one-time screen-recording prompt could not be exercised in that environment – Xvfb Chromium has no desktop capturer – and want a look on a Mac with a projector. `getDisplayMedia` and WebRTC exist in Firefox and Safari too, but nothing here has been tried there.
