...
// UTF-8, not the library's default: that one masks every code unit to a
// byte, so an address with a non-Latin-1 character in its path scanned to a
// different string and looked perfectly right on the slide. TextEncoder
// rather than the library's own UTF-8 table, which its ESM build does not
// carry; the live encoder (liveQrSvg in AUDIENCE_JS) and the spec set the
// same function, so every side draws one code.
qrCodeGen.stringToBytes = (s) => Array.from(new TextEncoder().encode(s));
...
function parseLecture(src) {
  ...
  const flushNoteBlock = () => {
    if (!noteBlock) return;
    // `> note: from 3` pins the block to a beat by number instead of by
    // where it stands. That is the escape hatch for the beats a position
    // cannot name: a diagram's steps are beats on the same counter but
    // they are inside one segment, so no `---` can sit between them. The
    // number counts advances on this slide, as `::: overlay from N` does -
    // 0 is the slide as it opens, 1 is after the first press.
    const pin = /^from\s+(\d+)$/i.exec((noteBlock.lines[0] || '').trim());
    if (pin) { noteBlock.from = Number(pin[1]); noteBlock.lines.shift(); }
    ...
    if (!inFence) {
      ...
      if (h2) {
        ...
        currentChunk = {
          ...
          speakerNoteFrom: pendingNotes.map(() => null),
...
/* Ink and accent grounds on paper: a bold keeps its accent on a light
   card and loses it on a dark or accent one, or it vanishes (measured:
   "Merke:" gone on an accent band, lead-ins dark red on black). */
:is(.overlay-card, .dock).ov-ink strong, :is(.overlay-card, .dock).ov-ink b,
:is(.overlay-card, .dock).ov-accent strong, :is(.overlay-card, .dock).ov-accent b,
...
function renderHelpOverlay(view, withEditor, withSouffleuse) {
  ...
  const souffleuseKeys = ['The prompter', [
    ...
    ['<kbd>Esc</kbd>', 'take the hint standing on the screen away – it also goes by itself, and the × on it does the same'],
...
/* A --- below the top level (BEAT_MARK). The marker itself is never shown;
   ...
   press. So the block is laid out at its final height from beat 0 and the
   words fade in where they were always going to be. Auto-fit measures the
   same height on every beat as a consequence. */
/* Three classes deep on purpose: the speaker's ghost has to win over this
   rule in turn, and the collapse rules (0-4-x) have to keep winning over
   the ghost, or the cockpit announces as next a block the projector never
   shows. */
.chunk .chunk-content .beat-mark, .chunk .overlay-layer .beat-mark, .chunk .dock .beat-mark { display: none; }
.chunk .chunk-content [data-beat-hidden], .chunk .overlay-layer [data-beat-hidden], .chunk .dock [data-beat-hidden] {
  visibility: hidden;
  opacity: 0;
  transition: opacity 0.4s ease, visibility 0.4s;
}
.chunk .chunk-content [data-beat-hidden] { pointer-events: none; }
...
/* tint - the card row's panel ground, the dock's default: a column on the
   slide's own paper needs an edge, and paper has none. Follows the theme
   through --ink, so a dark mode tints lighter. */
.dock.ov-tint { background: color-mix(in oklch, var(--ink) 5%, transparent); }
...
/* The same air on the picture side as on the frame side - with a card's
   1.05em there, the words sat against the photograph's edge - and neither
   is the slide's text inset: 139px twice out of a 30% column left 200px
   for the words and auto-fit shrank the type to 0.85 to make them fit.
   A share of the slide's width, so it neither follows the zoom nor eats
   the column. */
.overlay-card.ov-panel.ov-left, .overlay-card.ov-panel.ov-right { padding: var(--ov-pad-y) calc(var(--slide-w) * 0.045); }
.overlay-card.ov-panel.ov-left  { left: 0; }
.overlay-card.ov-panel.ov-right { right: 0; }
/* A column's width is a share of the slide, not a measure of type: in em
   it followed the zoom, and on a bare photo slide - where auto-fit has no
   body to measure and goes to its ceiling - a narrow panel came out at
   three quarters of the frame with a strip of picture left. The text
   measure inside it is still capped in em by the width word. */
.overlay-card.ov-panel.ov-left.ov-w-narrow,   .overlay-card.ov-panel.ov-right.ov-w-narrow   { width: 30%; }
.overlay-card.ov-panel.ov-left.ov-w-standard, .overlay-card.ov-panel.ov-right.ov-w-standard { width: 40%; }
.overlay-card.ov-panel.ov-left.ov-w-wide,     .overlay-card.ov-panel.ov-right.ov-w-wide     { width: 52%; }
.overlay-card.ov-panel.ov-left.ov-w-full,     .overlay-card.ov-panel.ov-right.ov-w-full     { width: 62%; }
.overlay-card.ov-panel.ov-left.ov-w-narrow > *,   .overlay-card.ov-panel.ov-right.ov-w-narrow > *   { max-width: 19em; }
.overlay-card.ov-panel.ov-left.ov-w-standard > *, .overlay-card.ov-panel.ov-right.ov-w-standard > * { max-width: 29em; }
.overlay-card.ov-panel.ov-left.ov-w-wide > *,     .overlay-card.ov-panel.ov-right.ov-w-wide > *     { max-width: 42em; }
...
/* A band takes the foot (or the head) of the frame, and a slide's words
   are centred in the frame's height, so on a slide with a heading and
   little else the band covered the heading's last line - measured on a
   divider whose two-line heading lost the word "Panel". The words move
   away from the band, up for a bottom band, down for a top one, and a
   the camera frames such a chunk as a whole (see the fit branch), or it
   would centre the moved words and push the band off the edge. */
.chunk:has(> .overlay-layer > .ov-panel.ov-bottom) > .chunk-content { align-self: start; }
.chunk:has(> .overlay-layer > .ov-panel.ov-top)    > .chunk-content { align-self: end; }
...
/* A panel that arrives on a beat wipes in from its edge rather than
   lifting like a card - and wipes rather than slides, because a hidden
   panel nudged past the frame overflowed the chunk by its nudge and
   auto-fit, which asks the chunk whether anything overflows, shrank the
   type to the floor on every slide with a panel held to a beat. */
.overlay-card.ov-panel { clip-path: inset(0); transition: opacity 0.5s ease 0.14s, clip-path 0.5s ease 0.14s, visibility 0.5s; }
.overlay-card.ov-panel[data-hidden] { transform: none; }
.overlay-card.ov-panel.ov-left[data-hidden]   { clip-path: inset(0 100% 0 0); }
.overlay-card.ov-panel.ov-right[data-hidden]  { clip-path: inset(0 0 0 100%); }
.overlay-card.ov-panel.ov-bottom[data-hidden] { clip-path: inset(100% 0 0 0); }
.overlay-card.ov-panel.ov-top[data-hidden]    { clip-path: inset(0 0 100% 0); }
...
/* chrome that would land on a column moves off it by the column's width;
   under a top band the badge and the note button move to the foot, which
   a top band never reaches (a band's height is not a number CSS has) */
.chunk[data-dock=top] > .chunk-num, .chunk[data-dock=top] > .annot-add { top: auto; bottom: var(--slide-pad-y); }
...
/* While it is being typed, the annotation is the slide. Pressing N used to
   ...
   backdrop chunk does, and the camera centres it (the annotEditingId branch
   of focusCamera), so the frame the room sees is the slide-high band around
   the chunk's middle - which is where the box is pinned, at the slide's
   height, whatever the chunk's: a short chunk is exactly that band, and a
   chunk taller than the frame (auto-fit off, a long exercise) gets a layer
   that is still the frame rather than one that runs off both edges of it.
   .chunk-content gives up position: relative for the duration, because the
   box is its child and would otherwise be sized to the measure; nothing
   else in there is positioned against it but the marginalia, which the
   scrim covers anyway. And the scrim is nearly the paper - the address
   someone is reading has no business competing with the slide behind it,
   which stays in the print. */
...
.chunk.annot-visible .annot-box {
  ...
  /* 7.5% a side, not the slide's 14%: the longest line may take 85% of the
     frame before the type shrinks (fitAnnotation reads the inner width) */
  padding: var(--slide-pad-y) 7.5%;
  border: 0;
...
.chunk.annot-visible .annot-textarea {
  ...
  line-height: 1.3;
  text-align: left;
  /* Below the floor size a line wraps rather than shrinking further, and it
     wraps between any two characters: fitAnnotation counts the rows a line
     takes as its length over the block's columns, and word-wrapping would
     take more rows than that and clip the last of them (the textarea is
     overflow: hidden and sized to the count). A tab or a double-width
     glyph still counts as one column; that is the one case the count can
     be short by. */
...
/* The two projector-command badges, BLANK and DEMO, share every rule; the
   ids keep only their stacking offsets below. */
.cmd-badge {
  position: fixed;
  bottom: 1.2rem;
  left: 50%;
...
/* ── Live demo (hotkey D) ─────────────────────────────────────────
   ...
   Above the figure overlay (30) and below everything that has to keep
   reporting during a demo: the touch rail (35), the laser pointer, the
   mode toast and the search panel (40), the address overlay and the
...
function applyRemoteState(payload) {
  ...
  try {
    ...
    flatChunks.forEach(c => {
      ...
      // Under the layer the size is fitAnnotation's, run once from applyState
      // below; autosize here would be a forced layout thrown away.
      if (ta.value !== v) { ta.value = v; if (!c.el.classList.contains('annot-visible')) autosize(ta); }
    ...
    if (annotEditingId !== remoteEditingId) {
      ...
      if (annotEditingId) {
        ...
        // fitAnnotation follows in applyState, which every apply reaches.
...
window.addEventListener('message', (ev) => {
  ...
  // The projection greets a window it has just adopted – a cockpit that
...
// The window the lecturer is looking at: the cockpit when there is one,
// otherwise the projection running alone. Where a badge or a toast lands.
...
// Live demo (D): the capture this window holds, whether the other window
// said it holds one, the loopback connection at either end, and the
// promise that its remote description is in. Declared here, next to the
// blank badge, because applyState reads them through applyDemoBadge before
// the demo section further down has run. The functions are in the "live
// demo" section.
let demoStream = null;
...
function applyDemoBadge() {
  if (!demoBadge) return;
  demoBadge.classList.toggle('hidden', !(demoOn() && lecturerScreen()));
}
...
function chunkBeats(el) {
  ...
  el.querySelectorAll('.reveal-segment, svg.psi-diagram, .beat-mark').forEach(node => {
    ...
    if (node.classList.contains('reveal-segment')) {
      ...
      //
      // The first rendered segment is normally the slide's opening state and
      // is no beat at all - but it can be a pinned one, when the segment
      // above it was emptied by a captured directive (a chunk that opens on
      // its backdrop and takes its words at beat 3). Without the second test
      // it was hidden by the FROM_SEL sweep at the foot of applyReveal and
      // managed by nothing: no entry in the beat list, so no next-mark in
      // the cockpit and no line in the cue list.
      const at = fromOf(node);
      if (segIdx++ > 0 || at != null) push({ type: 'seg', el: node, at });
      return;
    }
...
// the longest line stands in 85% of the frame and every line in its height,
...
// Same encoder, level, byte function and quiet zone as the build's qrSvg;
// the library's own SVG writer, because size is not the concern here that
// it is in a file every link of the lecture is inlined into. An address the
// largest symbol cannot hold (the library throws a string past ~2.3 KB at
// level M) is no code rather than an exception out of the input handler -
// that would have stopped the keystroke from reaching the peer and the
// draft from reaching localStorage until the address was shortened.
...
function fitAnnotation(entry) {
  ...
  const maxW = W;                 // 85% of the frame: the layer's own padding
...
function wireAnnotations() {
  flatChunks.forEach(({ el, id }) => {
    ...
    ta.addEventListener('input', () => {
      ...
      // Under the layer the height is fitAnnotation's; autosize would force
      // a layout only to be overwritten.
      if (el.classList.contains('annot-visible')) fitAnnotation(flatChunks.find(c => c.id === id));
      else autosize(ta);
...
function flowHeightProbe(el) {
  ...
  const flowKids = (node) => {
    ...
    for (const c of node.children) {
      ...
      // The overlay layer counts only when it holds a side panel, which is
      // looked through below; with cards or a band alone it is skipped as the
      // slide-high absolute box it is, or the fit would read it as content.
      const sidePanel = c.classList.contains('overlay-layer') && c.querySelector(':scope > .ov-panel.ov-left, :scope > .ov-panel.ov-right');
      if ((cs.position === 'absolute' || cs.position === 'fixed') && !c.classList.contains('dock') && !sidePanel) continue;
  ...
  // The overlay layer is looked through twice: the layer to its panels
  // (cards keep their cell and are not measured), a panel to its words -
  // so a bare photo slide whose only text is a panel is fitted to that
  // text rather than zoomed to the ceiling.
  const panelLevel = (layer) => {
    // Side panels only. A band sits at the foot (or head) of the frame, so
    // one extent from the words above it to the words in it is the slide's
    // height whatever the type size, and the fit ran to the floor. A band's
    // own height is its words' (snug) or a fixed share, and the words above
    // it are measured through .chunk-content as on any slide.
    const panels = [...layer.children].filter((k) => k.classList.contains('ov-panel')
      && (k.classList.contains('ov-left') || k.classList.contains('ov-right')));
    if (!panels.length) return null;
    const lv = levelOf(layer);
    // A panel is positioned against the layer's padding box, not inside its
    // padding, so the layer's own paddings are not part of the height.
    lv.padT = 0; lv.padB = 0;
    const levels = panels.map(levelOf);
    // A panel is the offsetParent of its words (it is absolute), so a
    // word's extent is the panel's offset plus its own, less the panel's
    // padding - the same arithmetic the closure below does one level up.
    lv.spanFn = () => {
      let top = Infinity, bottom = -Infinity;
      panels.forEach((p, j) => {
        const s = span(levels[j]);
        if (!s) return;
        const t = p.offsetTop + s.top - levels[j].padT, b = p.offsetTop + s.bottom + levels[j].padB;
        if (t < top) top = t;
  ...
  const inner = outer.flow.map((c) => (c.classList.contains('chunk-content') || c.classList.contains('dock') ? levelOf(c)
    : c.classList.contains('overlay-layer') ? panelLevel(c) : null));
  ...
  return () => {
    ...
    for (let i = 0; i < outer.flow.length; i++) {
      ...
      const lvl = inner[i];
      const s = lvl && (lvl.spanFn ? lvl.spanFn() : span(lvl));
      if (s) {
    ...
    // A band at the head or foot is not in the span (see panelLevel) but
    // it takes its height from the frame all the same, so the words have
    // that much less: a two-line divider heading lost its second line under
...
// A capture is never larger than a projector shows; a Retina or 5K screen
// captured at its own size is a copy per frame nobody sees the benefit of,
// and on the loopback path a frame size the software encoder cannot keep
// up with at all.
const DEMO_MAX_W = 2560, DEMO_MAX_H = 1600;
...
function detachDemo() {
  if (demoVideo) demoVideo.srcObject = null;
  if (demoOverlay) demoOverlay.classList.add('hidden');
  document.body.classList.remove('demo-live');
  if (demoPc) { demoPc.close(); demoPc = null; }
  demoRemoteReady = null;
}
...
function stopDemo() {
  endDemoLocally();
  sendToPeer({ type: 'demo', source: VIEW, action: 'stop' });
...
async function startDemo() {
  ...
  // The cockpit never shows the picture itself, so without a projection
  // window a capture would run into nothing while the badge said DEMO.
  if (VIEW === 'speaker' && !hasLivePeer()) {
    flashMode('no projection window – it opens with S from the audience view');
    return;
  }
  demoPending = true;
  let stream;
  try {
    stream = await navigator.mediaDevices.getDisplayMedia({
      video: {
        width: { max: DEMO_MAX_W }, height: { max: DEMO_MAX_H },
  ...
  demoStream = stream;
  // Chrome's own "stop sharing" bar ends the track; that ends the demo too.
  track.addEventListener('ended', () => { if (demoStream === stream) stopDemo(); }, { once: true });
  applyDemoBadge();
  if (VIEW === 'speaker') deliverDemo(stream);
  else attachDemo(stream);
  sendToPeer({ type: 'demo', source: VIEW, action: 'live' });
  flashMode('demo live');
}
// D while the picker is still open is neither a start nor a stop.
function toggleDemo() {
  if (demoPending) return;
  if (demoOn()) stopDemo();
  else startDemo();
}
// A peer that has just appeared – a cockpit opened with S, a projection
// back from a reload – is handed the running demo again. Called on hello.
function demoAnnounce() {
  if (!demoStream) return;
  if (VIEW === 'speaker') deliverDemo(demoStream);
  sendToPeer({ type: 'demo', source: VIEW, action: 'live' });
}
// Same origin: hand the stream over as an object. Otherwise the loopback
// connection. A same-origin peer without the hook is a projection built
// before the demo existed, and that is said rather than silently
// re-routed to an offer it would never answer.
function deliverDemo(stream) {
  if (DEMO_DIRECT) {
    if (peer && typeof peer.psiDemoAttach === 'function') peer.psiDemoAttach(stream);
    else { endDemoLocally(); flashMode('demo: the projection window did not answer – rebuild it'); }
    return;
  }
  demoSendOffer(stream);
}
function demoNewPc() {
  // No ICE servers, deliberately: both ends are this machine, and a STUN
  // server here would make every D press reach the internet.
  const pc = new RTCPeerConnection();
  demoPc = pc;
  demoRemoteReady = new Promise((resolve) => { pc.psiRemoteReady = resolve; });
  ...
  pc.onconnectionstatechange = () => {
    ...
    if (pc.connectionState === 'failed' || pc.connectionState === 'closed') {
      endDemoLocally();
      flashMode('demo: connection lost');
    }
  };
  return pc;
}
function demoSendOffer(stream) {
  ...
  // internet and blurs a terminal the moment it scrolls. Set before the
  // offer and measured to survive negotiation in Chromium 141.
  ...
  pc.createOffer()
    ...
    .catch(() => { if (demoPc === pc) { endDemoLocally(); flashMode('demo: could not connect'); } });
}
// A fresh offer replaces whatever connection was up. Every await checks the
// connection is still the current one: a quick stop and start can leave an
// older handshake in flight, and it must neither speak nor toast.
async function demoReceiveOffer(m) {
  if (typeof RTCPeerConnection !== 'function') return;
  detachDemo();
  const pc = demoNewPc();
  pc.ontrack = (ev) => { if (demoPc === pc) attachDemo(ev.streams[0] || new MediaStream([ev.track])); };
  try {
    ...
    if (demoPc === pc) { detachDemo(); flashMode('demo: could not connect'); }
  }
}
async function demoReceiveAnswer(m) {
  const pc = demoPc;
  if (!pc) return;
  try { await pc.setRemoteDescription({ type: 'answer', sdp: m.sdp }); } catch (e) { return; }
  if (demoPc === pc) pc.psiRemoteReady();
}
// A candidate can arrive while setRemoteDescription is still pending on
// this side; it waits on the promise until the description is in.
function demoReceiveIce(m) {
  const pc = demoPc, ready = demoRemoteReady;
  if (!pc || !ready) return;
  ready.then(() => { if (demoPc === pc) return pc.addIceCandidate(m.candidate); }).catch(() => {});
}
// Closing or reloading the capturing window ends its tracks; the other
// window has to hear that, or the projection keeps a frozen last frame.
// Only the message – the browser tears down the rest.
window.addEventListener('pagehide', () => {
  if (demoStream) sendToPeer({ type: 'demo', source: VIEW, action: 'stop' });
});
...
body[data-view=speaker] :is(.reveal-segment[data-hidden], .chunk [data-beat-hidden])[data-next]::before {
  content: '';
  position: absolute;
  inset: -5px;
  pointer-events: none;
...
body[data-view=speaker] :is(.reveal-segment[data-hidden], .chunk [data-beat-hidden])[data-next]::after {
  content: 'next';
  position: absolute;
  top: -1.5em; right: 0;
  font-family: var(--sans-font);
...
/* A nested beat keeps its box on both screens (see the hide rule in the
   ...
   strength, in the flow, the same height in the cockpit as on the
   projector - which the laser pointer relies on. Positioned only so the
   hatch and the label have something to anchor to. */
body[data-view=speaker] .chunk [data-beat-hidden][data-next] {
  visibility: visible;
  opacity: 0.5;
  position: relative;
  outline: 2px dashed var(--emph);
...
/* A list dissolved into a card grid has no box to hatch; the items carry
   the outline instead. */
body[data-view=speaker] .cards > :is(ul, ol)[data-beat-hidden][data-next] { position: static; outline: 0; }
...
body[data-view=speaker] .cards > :is(ul, ol)[data-beat-hidden][data-next] > li { outline: 2px dashed var(--emph); outline-offset: 3px; }
...
#cue-where {
  ...
  align-items: center;
  gap: 1em;
  padding: 1.2vh 2.4vw 0 calc(2.4vw + 2.2em);
...
#cue-rail {
  ...
  align-content: start;
  /* The size everything in the rail is measured against. Generous on
     purpose: this is read at a glance while talking, from a laptop that is
     further away than a screen being worked at - and adjustable, because
     how far away that is differs per room and per person. */
  font-size: calc(clamp(17px, 2.7vh, 32px) * var(--cue-scale, 1));
}
...
.cue-entry.later { color: var(--ink-soft); font-size: 0.92em; }
...
function scrollPreviewToActive(smooth) {
  // In cue-card mode the mirror stands in for the current thumbnail, so it
  // is what has to end up in the middle of the strip.
  const el = cueOn() ? stageCell : previewStrip.querySelector('.preview-slot.current');
  if (!el) return;
...
// { id, beat, card } - the chunk, the number of advances it has taken,
...
// that thumbnail - so the left column is one film strip with the real
// projection in the middle of it, rather than two copies of one slide only
// one of which is laid out the way the room sees it. populatePreviewStrip
// replaces every child of the strip, so this runs after it and after every
// re-mark.
function cuePlaceStage() {
  if (!cueOn()) return;
  const cur = previewStrip.querySelector('.preview-slot.current');
...
// Card size, per user and across lectures like the notes-pane zoom and the
// strip orientation: how far the laptop stands from the lectern is a
// property of the room, not of the deck.
function applyCueScale(v) {
  document.documentElement.style.setProperty('--cue-scale', String(v));
  try { localStorage.setItem(CUE_SCALE_KEY, String(v)); } catch (e) {}
}
function cueScale() {
  const v = Number(document.documentElement.style.getPropertyValue('--cue-scale'));
  return v > 0 ? v : 1;
}
...
// further down, so a restore at this point ran into their temporal dead
// zone, and the try that guards localStorage swallowed the ReferenceError
// whole: the cockpit came up in cue-card mode with an empty rail, and the
// first arrow key filled it. Reading localStorage is what needs the try;
// applying the mode does not, and wrapping both hid a defect for a release.

// The cards of a chunk, keyed by the number of advances after which they are
// said - the same number an overlay's from N counts, and the same one
// applyReveal calls consumed. 0 is the slide as it opens.
//
// A note pins itself with a note-from line and is filed under exactly that.
...
// advanced); consumed is pos - 1, the number of presses this slide has
// taken, which is what every beat and every card is filed under.
function cuePosition(entry) {
  const total = countSegments(entry.el);
  const pos = Math.max(1, revealed[entry.id] ?? 1);
  // Every beat, pinned or not. The filter that used to stand here dropped
  // anything carrying an at-number, written when only a marker inside a
  // pinned overlay could carry one and the cue list had nothing to say about
  // those. A pinned segment carries text, so dropping it shifted segAt for
  // every segment after it and left the last one falling off the end - the
  // cards for those segments were filed on beat 0 and shown as the slide
  // opened. cueEntries decides per type what it can name; that is the place
  // for the question, not here.
  const beats = chunkBeats(entry.el);
  return { pos, consumed: pos - 1, total, maxC: Math.max(0, total - 1), beats };
}
...
function cueChunkTitle(entry) {
  ...
  // A column's divider carries no chunk heading - what names it is the
  // column, which is also what the room is about to read on it.
  if (entry.el.dataset.tag === 'section') return cueColumnTitle(entry) || 'the next part';
...
// The column as one list: for every advance this slide takes, the cards said
// on it and then the press that leaves it - a reveal, a diagram step, a
// nested beat - and at the end the next slide. That is the order the room
// experiences, so it is the order to read down.
function cueEntries(entry) {
  const { consumed, pos, total, maxC, beats } = cuePosition(entry);
  const cards = cueCardsFor(entry.id, beats, maxC);
  const segTotal = beats.filter(b => b.type === 'seg').length;
  const out = [];
  let segN = 0;
  for (let c = 0; c <= maxC; c++) {
    (cards.get(c) || []).forEach((card, k) => out.push({ type: 'card', c, k, card }));
    // c is a 0-based index and an entry says it arrives on press c + 1, so
    // an unpinned beat matches its own pos while a pinned one - which arrives
    // when consumed reaches its number - matches that number minus one.
    // cueCardsFor files the same beats one scale up; the two must not be as
    // the same number.
    const b = beats.find(x => (x.at != null ? x.at - 1 : x.pos) === c);
    if (b && b.type === 'seg') {
      out.push({ type: 'step', at: c + 1, k: 'reveal ' + (++segN) + '/' + segTotal, what: cueText(b.el, 120) });
    } else if (b && b.type === 'diag') {
      // The author's own name for the beat if the step carries one - it
      // says what the picture is about to do, which "step 2" does not.
      const nm = (b.d.data.names || [])[b.step - 1] || '';
      out.push({ type: 'step', at: c + 1, k: 'figure · step ' + b.step, what: nm });
    } else if (b) {
      out.push({ type: 'step', at: c + 1, k: 'beat', what: cueText(b.els[0], 120) });
    } else if (c < maxC) {
      // An advance the beat list does not carry: an overlay held by from,
      // a backdrop frame. It is still a press.
      out.push({ type: 'step', at: c + 1, k: 'advance', what: '' });
    }
  }
  const nxt = flatChunks[state.activeIdx + 1];
  const nxtK = !nxt ? 'end'
    : nxt.el.dataset.tag === 'section' ? 'next part'
    : 'slide ' + (state.activeIdx + 2);
  out.push({ type: 'chunk', k: nxtK, what: nxt ? cueChunkTitle(nxt) : 'the last slide' });
  // said or not, and the first that is not is the cursor
  let cur = -1;
  out.forEach((e, i) => {
    e.done = e.type === 'card' ? (e.c < consumed || (e.c === consumed && e.k < cue.card))
           : e.type === 'step' ? e.at <= consumed : false;
...
// Called from onActiveChange and onStateChange, so the cursor follows a jump,
// a TOC pick, a remote apply - anything that moves the deck. The two consume
// hooks bind too: at boot nothing has moved yet, so a first Space would
// otherwise count against a cursor belonging to no chunk.
function cueSync() {
  const entry = flatChunks[state.activeIdx];
  if (!entry) return;
  cueBind(entry);
...
// Restore the saved arrangement, now that every binding cueRender reaches
// exists. The read is guarded, the apply is not - see the note beside
// toggleCueMode.
let cueModeSaved = null;
try { cueModeSaved = localStorage.getItem(CUE_MODE_KEY); } catch (e) {}
if (cueModeSaved === 'on') applyCueMode(true);
...
if (SOUFFLEUSE && window.psiWatch) {
  ...
  // "off - no OPENROUTER_API_KEY" up, and the idle that followed the
  ...
  if (souffBtn) souffBtn.addEventListener('click', () => souffToggle());
  ...
  // The interim line is built in the next slice. The adapter reports
  // interims all the same: what decides whether they are shown is not the
  // ear.
  function souffInterim() {}
  ...
  viewHooks.onActiveChange = () => { souffPrevActive(); souffMoved(); };
  ...
  souffWatch.on('souffleuse-hint', (m) => {
    ...
    const ms = high ? 25000 : 15000;
    // The strip is the next slice. Until it stands, the centre toast is the
    // only thing in this window that shows a sentence for long enough to
    // read it - and it cannot be dismissed, so the only way one goes away
    // is by fading, which is what the sidecar is told.
    flashCenter(String(m.text || ''), { warn: high, duration: ms });
    setTimeout(() => souffWatch.ask('souffleuse-dismiss', { hintId: m.hintId, how: 'fade' }), ms);
  });

  souffWatch.on('souffleuse-cue', (m) => {
    if (!SOUFFLEUSE.cues) return;
