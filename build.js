...
/* An expansion is half of a two-column grid, which needs two columns' worth
   of room. Under this width there are not two, so the pane covers the slide
   instead of being squeezed beside it - the same trade the figure overlay
   makes, and for the same reason: a card you can read beats a layout you
   can see all of. */
@media (max-width: 900px) {
  .chunk.expanded { grid-template-columns: minmax(0, 1fr); }
  .chunk.expanded .exp-body.on {
    position: fixed;
    inset: auto 4vw 12vh;
    max-height: 62vh;
