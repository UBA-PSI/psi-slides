...
@media (pointer: coarse) {
  ...
  /* In the cockpit the bottom edge is already spoken for: the thumbnail strip
     and then the footer. The rail clears both by reading the same two numbers
     the grid does, so a dragged strip carries it along. With the strip moved
     to the right edge (Shift-V) there is no bottom row to clear, and
     --preview-h is *removed* rather than set to zero - so the fallback would
     lift the rail 22vh off the floor for a strip that is not there. Hence the
     second rule rather than one clever expression. */
  body[data-view=speaker] #touch-controls {
    bottom: calc(2.2rem + var(--preview-h, 22vh) + 0.5em);
  }
  body[data-view=speaker].preview-right #touch-controls {
    bottom: calc(2.2rem + 0.5em);
  }
}
...
function focusCamera(instant = false) {
  ...
  // The open expansion pane, but only while it is part of the chunk's own
  // layout. Under 900px it is position: fixed, and a fixed element has no
  // offsetParent chain up to the stage - getOffset then hands back viewport
  // coordinates, and the camera built from them translated the deck to
  // (-4320, -40850) on an 800px window. The card sat where fixed positioning
  // put it and looked right; the slide it is supposed to cover was thirty-
  // seven thousand pixels away, so it covered an empty page. A pane that
  // frames itself wants no camera of its own: fall through to the chunk.
  const expPane = (openExp && openExp.chunkIdx === state.activeIdx)
    ? entry.el.querySelector(\`.exp-body[data-exp-body="\${openExp.expIdx}"]\`)
    : null;
  const paneInFlow = expPane && getComputedStyle(expPane).position !== 'fixed';
