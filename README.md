...
**The live prompter (`--prompter`) is new in 2.0.0, and it is off unless you switch it on.** In a lecture built with `--watch --prompter`, the cockpit listens to the room while you talk, and when something needs saying it shows a hint of at most twelve words on a strip across the bottom of its copy of the slide: you are behind time, a point stays abstract although your slides have an example for it, what you just said contradicts your own slide, you are speaking too fast, or your notes planned something on this slide that you have walked past. It can also add a cue card to a slide still to come, the last slide included, when you said something in passing that is worth saying again there. (The cockpit's `K` mode shows your speaker notes as cue cards; in the ordinary layout the added card appears on the strip when you reach that slide.) The build measures the tempo rather than leaving the model to guess it: a transcript carries no speaking rate, hesitation or silence, so the build counts words a minute, filler sounds and the longest pause and hands the model the figures. Chrome's speech recognition often drops filler sounds, so a count of zero proves nothing.

`Shift`-`S` in the cockpit switches the prompter on and off, and nothing listens until you press it. For the first minute a faint line under the strip shows the words the speech recognition picks up, so you can see that it works; after that the same line says when the prompter last asked the model, because a prompter with nothing to say looks the same as one that has stopped. It needs Chrome, because the listening is Chrome's own speech recognition (the Web Speech API); it needs `--watch`, because the cockpit hands what it hears to that process and the process asks the model; and it needs an `OPENROUTER_API_KEY` in the environment, a key for openrouter.ai, the service that passes the request on to a language model. Without the key the prompter starts switched off and only writes down what it heard.

**What the prompter sends is text.** The transcript and the lecture's text, speaker notes included, go to openrouter.ai and from there to the company that runs the model. The prompter sends no audio. The speech recognition is Chrome's, though, and Chrome sends the audio to Google unless it can run the recognition on your own device; the cockpit says which of the two you are getting. Nothing of this reaches the projection, nothing is written back into `source.md`, and the key is read by the build and never appears in the built HTML. To try the prompter without calling a model, `--prompter-dry-run` does everything except that call, and it needs no key; with recognition on the device, nothing leaves the machine at all. A run makes at most `calls-per-hour` calls in any hour (360 unless the `prompter:` block says otherwise), and the model named in `prompter: {model: …}` is the one you pay for – check its price before you rehearse with an unfamiliar one.

**Each run writes a log beside `source.md`** (`prompter-<date>.jsonl`): what it heard, which hints it showed, and what it wanted to say but its rules stopped, with the rule that stopped it. `--prompter-replay` reads a log back and prints, answer by answer, what the model proposed and what today's rules would do with it. Beside the log, `prompter-<hash>.prompt.txt` holds the lecture's text exactly as the model got it. The log holds your spoken words verbatim. This repository's `.gitignore` covers both files; a lecture kept in a repository of its own needs `prompter-*.jsonl` and `prompter-*.prompt.txt` in its own `.gitignore`, and the build prints the log's full path on every start.

**The microphone hears the room too**, not only you: switch it off before a question round, or tell the room it is on.
...
- `Shift`-`S` in the speaker window switches the live prompter on and off, in a lecture built with `--watch --prompter`. The `◌ prompter` button in the footer is the same switch, and `Shift`-clicking it shows the last ten hints and cards. `Esc` removes the hint currently on the strip.
