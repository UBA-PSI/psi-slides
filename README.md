...
**The live prompter (`--prompter`) is new in 2.0.0 and opt-in.** Built with `--watch --prompter`, the cockpit can listen to the room while you talk and whisper back at most twelve words when something needs saying – you are behind time, the example is missing, what you just said contradicts your own slide, you are speaking too fast, or your own notes planned something on this slide that you have walked past – and it can lay a cue card into a slide that is still to come, the conclusion included, for a sentence worth keeping that you said in passing. The tempo half is counted rather than guessed: a transcript carries no speaking rate, no hesitation and no silence, so the build measures words a minute, filler sounds and the longest gap and hands the model the numbers (with the caveat that Chrome's recogniser often strips filler sounds, so a count of zero proves nothing). `Shift`-`S` in the cockpit switches it on and off, and nothing listens until you press it. For the first minute it shows you the words it is hearing, so you can see the ear works, and after that a faint line under the strip says when it last asked the model - because a prompter doing its job correctly is silent, and silence looks like a prompter that has died. It needs **Chrome** (the ear is the Web Speech API), it needs `--watch` (the cockpit reaches it over the watch socket), and it needs an `OPENROUTER_API_KEY` in the environment; without the key it starts disabled and only writes down what it heard.

**What leaves the machine, as text.** Speech recognition runs on your own device where Chrome can do it and through Google where it cannot – a badge in the cockpit says which of the two you are getting. The transcript and the deck itself, speaker notes included, go to openrouter.ai to be answered. No audio is sent anywhere, nothing of this reaches the projection, nothing is written back into `source.md`, and the key is read in Node and never appears in the built HTML. Each run writes a log beside `source.md` (`prompter-<date>.jsonl`, gitignored) so you can read afterwards what it heard, what it said, and what it wanted to say and was not allowed to – `--prompter-replay` reads that log back and prints, answer by answer, what the policy would do with it now. Beside the log, `prompter-<hash>.prompt.txt` is the deck exactly as the model gets it, so you can read that too. If you would rather try the whole thing without sending anything anywhere, `--prompter-dry-run` runs the ear, the clock and the policy and skips the one call – it needs no key. And the microphone hears **the room** too, not only you: switch it off before a question round, or tell the room it is on.
...
- `Shift`-`S` in the speaker window starts and stops the live prompter, in a lecture built with `--prompter` – the `◌ prompter` button in the footer is the same switch, and `Shift`-clicking it shows the last ten things it has said. `Esc` takes the hint currently standing on the strip away.
