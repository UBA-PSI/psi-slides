...
- `::: cards N` – N equal containers in a row. Distinct from `::: cols N`, which is one text flow balanced across N tracks and may spill a paragraph from one column into the next. Use `cols` for an argument that runs long and `cards` for a comparison the room should be able to count.
