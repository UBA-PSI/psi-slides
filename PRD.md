...
- **Room for what has not arrived.** A top-level segment closes up before its beat and the chunk grows by a block per press – the behaviour since 1.0.0 and the default, `style: {reveal: grow}`. `style: {reveal: hold}` lays every chunk out at its final height from beat 0, deck-wide what a nested beat does anyway. Not the default, because it moves every existing deck's slides.
