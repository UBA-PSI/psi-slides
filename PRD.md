...
**Chunk.** The atomic unit of lecture content, rendered as one slide. Has a stable ID, an optional heading, a body, an optional structural type (`title`, `closing`, `outline`, `principle`, `statement`, `definition`, `example`, `question`, `figure`, `exercise`, `free` – §2.1), a width class, optional reveal separators (`---`) inside the body (§4.6), optional expansions, and an optional author-editable annotation slot. The width class determines the slide's *internal* text-column max-width, **not** the frame size.

**Expansion.** Detail content (deeper explanation, worked example, answer to a question) that lives with its parent chunk, reached via a chevron affordance in the bottom-right of the slide. When opened, the slide's internal layout splits into two columns – content on the left, expansion on the right – without leaving the slide frame. The chip carries the author's own label (`::: expand Proof` reads `Proof`); it used to carry a two- or three-letter abbreviation derived from it, which a room could not decode.

**Annotation slot.** One author-editable text area per chunk, used for live speaker marginalia during a lecture (not for source-authored references – those belong in a `Ref` expansion). Hidden by default; revealed only when the speaker activates it (`N` key or click on the `+ note` affordance). **While it is being typed, the annotation is the slide**: the box grows to the frame over a near-opaque scrim and the type is sized from the text alone – the largest size at which the longest line stands in 70% of the frame and every line in its height, capped at three slide sizes – so a single word stands large and centred, several lines stand as a left-aligned block of their longest line's width, and ASCII art keeps its columns (the face is monospace). The last `http(s)://` address in the text gets a QR code above the words, sized by what the words leave. Nothing of this travels in the sync snapshot: both windows derive the same picture from the same string. `Esc` returns focus to the slide, and the annotation rests as a small margin note beside the text column.
...
4. **Camera.** The camera **translates only**; there is no `transform: scale()` at the camera level. On chunk change, the stage translates to place the active slide centered in the viewport. On annotation activation the chunk takes the slide's height and the camera centres it, which is what makes the annotation layer the frame.
...
6. **Neighbor behavior.** Two modes, set by the `neighbours:` frontmatter key: `dim` (neighbors always at reduced opacity, `calc(1 - var(--dim) * 0.96)` with `--dim: 0.86`, about 17%) and `hidden` (always fully transparent, the keynote answer). Default: **`dim`** – calibrated authoring runs found constant peek preferable to motion-only peek, and a `fade-after-settle` mode was specified and never built. `transition: cut` and `transition: fade` imply `hidden` unless the author writes `dim`, because under them the camera never travels past a neighbour.
...
| `outline` | The running agenda: every part of the lecture (the headed columns, in source order) listed, the current one live, none live before the first. The same list `section: outline` draws on a divider, as a slide the author places – usually right after the cover. Unlike a divider it prints. Budget 40 words, because its words are the column headings, which it does not own. |
...
| `statement` | A slide that is a few lines of large type and nothing else. The heading is the first line and every top-level paragraph is another one, all at the same size, the same weight and in the ink colour; `---` between two of them makes them arrive one press apart, and `.center` moves the whole run – heading included – onto a centre axis. One paragraph written *entirely* in italic is the type's second register, the quiet line: half the size, the body weight, `--ink-soft`, in both views – the definition a claim answers or the source under it, as against a loud line with a stress mark on it, which is what an italic run *inside* a paragraph stays. No eyebrow in either view and no rule above, because both turn an utterance into a labelled specimen. Budget 80 words, as `principle`. The collapse never abridges these paragraphs: they are the slide, so `splitSentencesIn` skips them the way it skips a `::: slide` block. It opens centred on the frame, the way a slide that is one picture does, and `{.top}` keeps it at the head instead. |
...
subtitle: What a crowd buys you, and what it does not
presenter: Dominik Herrmann
affiliation: University of Bamberg
contact: psi.uni-bamberg.de
notice: Slides go up after the lecture.
info: |
  2026-04-21, Bamberg
  introsec-ss26 – lecture 07
course: introsec-ss26
lecture: 07
# optional cover keys – see §4.4
cover: masthead
cover-ground: ink
...
auto-fit: shrink
slide-numbers: off
print-slide-numbers: horizontal
editor: both
---
...
**Viewer defaults.** Nine optional frontmatter keys pin how a lecture *opens*: `font` (serif/sans/mono), `theme` (the six accent and phosphor names), `collapse` (topic-bold/none, the `C` toggle), `auto-fit` (true/false/shrink, the `#` cycle), `slide-numbers` (vertical/horizontal/off, the `L` cycle), `print-slide-numbers` (the same three, for the document views), `editor` (both/speaker/none), `note-button` (on/off, the `M` toggle – the `+ note` affordance in the slide's left gutter, which `N` reaches whether or not it is drawn) and `neighbours` (dim/hidden – whether the slide before and after show through at about 17%, which is the camera-panning-a-column reading the live view is built on, or not at all, which is what a keynote wants). `editor` is not a look but a payload – whether the live views carry the diagram editor – and it goes through the same machinery because the failure mode is the same: a typo would otherwise cost the lecture its editor without anything on the page saying so. Precedence is one sentence: **a key that is present wins over the reader's stored preference; a key that is absent leaves that preference alone.** Lectures that pin nothing therefore behave exactly as before – font, theme and slide numbers keep following the reader across lectures, which is the point of storing them globally – while an author who has designed a look gets it without asking anyone to press keys. An unknown value fails the build with the list of valid ones: a silently dropped setting is indistinguishable from a setting nobody wrote.

Three of those need their own sentence.

**`auto-fit` has a third mode.** `true` sizes every slide to the screen, which means it grows a short one as readily as it shrinks a long one – the ceiling is the tool's global maximum. `shrink` runs the same fit with the *lecturer's own zoom* as the ceiling, so it can only ever take size away: a slide that fits is left at exactly the zoom that was set, and a slide that does not is shrunk until it fits. That is what most lectures want from the feature, and it is why the key stopped being a boolean.

**`slide-numbers` reaches the document views too**, since a document has no keyboard, and `print-slide-numbers` is there for the case where the two should differ – a page read at arm's length and a projection read across a room are not obliged to want the same marker. Its default is a deferral rather than a value: absent, it *is* whatever `slide-numbers` resolved to.

**`slide-numbers` now defaults to `horizontal`**, where up to 1.0.0 it defaulted to `vertical`. It is the one viewer default whose own change moves what an existing deck renders, and it was taken deliberately: the stacked form sets each digit on its own line, so slide 10 reaches the room as a 1 above a 0. The old rendering is one line of frontmatter and there is no compatibility switch beside it.

**Rules:**
- `# Heading` = column title.
- `## type: Chunk heading` = chunk, with structural type prefix. Type is optional; width class and ID are attributes.
- `## title: {#title}` = cover slide. The heading text after `title:` is ignored – the cover renders from frontmatter, so the heading is left empty by convention and only the `{#title}` id is needed. The body may also be empty; a non-empty body overrides `info`.
- **The cover's words come from seven keys in four ranks.** `title:` and `subtitle:` are the pair, and `style: {headline: …}` says which of the two is the loud one (§4.2). Under them the credits: `presenter:` carries the name, `affiliation:` a quieter line set directly beneath it, and `contact:` and `notice:` share a single row along the foot, the address flush left and the notice flush right. `info:` keeps the venue, the date and the course line. The split is by job – a presenter and an institution introduce the speaker, an address and “the slides go up afterwards” answer the room, and a date is neither. Before the ranks existed everything but `presenter:` went into `info:`, which set the line qualifying the speaker's name exactly like the line giving the date. A `## closing:` slide repeats as much of the block as `closing-credits:` asks for: `none` (the default), `contact` (the foot row alone) or `cover` (all of it).
...
**A deck closes the arc with `## closing:`, and it is a type rather than a second `title:` chunk.** The last slide is drawn in whatever composition `cover:` names, so the room sees the shape it started with – but it carries the author's own words and, by default, neither the presenter line nor the `info` block. Those two say who is talking and where, which the room learned an hour ago, and setting them again in the same composition is a slide that reads as a mistake in the deck rather than as an ending. That argument never covered the one line a last slide is most often asked to carry, which is where the slides can be found, so `closing-credits:` is graded rather than boolean: `none` is the default and what is described here, `contact` brings back the foot row alone (`contact:` and `notice:`), and `cover` brings back the whole credit block. `cover` is the same reserved word `closing-image:` uses and for the same reason – it names *which* credits, where a word like `same` would only say that there are some. The composition is inherited and the content is written, which is what makes it the same shape without being the same slide. Three things ruled out the alternatives. A title chunk's heading is *ignored* (§3) because the cover renders from frontmatter, so a closing slide could only get its own words by making the heading mean something on the second occurrence, which is a positional exception to a frozen rule; `lint.js` already warns that a second `title:` chunk does not render, so the two spellings would contradict each other; and a frontmatter key could only ever recombine the cover's own fields, which is exactly the repeat this slide exists not to be. The four picture compositions draw their type alone here unless the author asks otherwise: a closing slide never reaches for `cover-image` by itself, because taking the cover's picture unasked is precisely the repeat this slide exists not to be. `closing-image:` is the asking, and it is `cover-image`'s counterpart – `closing-image: cover` ends the deck on the picture it opened with, so the filename is written once; any other value names a different picture in the same three forms `cover-image` takes. It draws into the composition's own picture slot through the same code, so the last slide divides the frame the way the first one did, and it is why the closing slide takes `cover-ratio` when, and only when, it has a picture to divide it for. A `::: backdrop` on the chunk is a different thing and still wins over it: a full-bleed ground behind the type, available on all ten compositions and the only way to put a photograph behind the six that have no picture slot. Set on one of those six, or set to `cover` with no `cover-image` to reuse, or set in a deck with no `closing:` chunk, `closing-image` fails the build.
...
**A figure can walk itself.** `::: draw autoplay 1200` advances the figure's own steps on a timer once the slide is on screen, one delay for every step, so a cover figure moves while the room files in. It calls the same advance the Space key calls, so it is one counter and the speaker view follows for free; it runs in the audience only, and the first key, pointer or wheel event **on that slide** retires the clock for it, because a lecturer who has touched the deck has taken over.

**A slide is a frame, and the frame can carry more than a text column.** Three directives, all additive:

- `::: backdrop <ref> {…}` (`::: backdrop dusk {.cover .invert}`) – a full-bleed image behind the whole slide, on any chunk. Its scrim is the theme's own paper by default, so ordinary ink stays legible over a photograph in all seven themes; `invert` turns the slide's ink light instead.
- `::: overlay {…}` … `:::` (`::: overlay {.bottom-left .ink}`) – a grounded text block laid over the slide, placed in one of nine cells. The ground is the point rather than decoration: text laid straight onto a photograph is unreadable at the back of a room.
- `::: dock {…} from N` … `:::` (`::: dock {.left .every}`) – the overlay's vocabulary with the other layout contract. An overlay is a *layer*: it lies over the slide and takes nothing from the text. A dock is *frame*: a left or right dock is a column the full height of the slide and the text column narrows beside it, a top or bottom dock a band across the whole width with the text above or below it. Under a `#` heading, `.every` puts the same dock on every chunk of the part, and a `#id` link in it lights the item the room is on – a running table of contents, a rule that stays in view, a remark that arrives on a beat into a track kept free for it. The form decision is the distinction itself: one word choosing between *covers the text* and *moves the text* was too much weight for a class on `::: overlay`, so it is a directive of its own.
...
- `::: side` takes an optional ratio (`::: side 2:1`) and an optional anchor tail (`::: side {.middle}`, `::: side 2:1 {.middle}`). The ratio turned out to be most of what *figure beside the text* needed; figure *above* or *below* needs nothing at all, being document order. The anchor is the one question the ratio left: a short pane sits at the top of its half and leaves the rest blank, which beside a tall figure is most of a slide. `anchor: top | middle`, the same two words a `::: cards` row uses for the same question, and `top` is the default, so a bare `::: side` draws exactly what it always drew. It is the **block's** switch and not each pane's, which is a fact about grids rather than a simplification: a row is as tall as its tallest item, so the tall pane already fills it and centring can only move the short one. Print stacks the panes and reads neither.
...
**A lecture can set its own type, within bounds.** The `style:` block carries nineteen keys: `headings` (`auto` keeps the per-type treatment; `left` overrides all of it; `off` takes it off the projection), `rules` (the hairlines above principle and definition chunks), `labels` (the generated type word), `link-codes` (the mark after an external link), `wrap`, `blocks`, `hyphenate`, `print-body` (whether the printed document is set in the serif or the sans), `bold` and `print-bold` (how a `**bold**` phrase looks on the two screens and on paper), `neutrals` and `print-neutrals` (what hue the greys carry), `headline` and `caps` (the title pair's two treatments), `code` (the look of an inline code span), and `heading-scale` / `body-scale` / `display-scale` as multipliers on the tool's own scale. Those three are bounded to 0.6–1.8, because outside that range the collapse mode, the code-width clamp and the auto-fit camera stop agreeing with each other and the result is not a look but a bug report.

The nineteenth is `figure-type`, and it answers the same question for a drawing that `body-scale` answers for prose: a `::: draw` is sized so that one base label is one em of the text it stands in, times this number. Its default is 1 – on a slide a label is a word the back row reads, and there is no running text beside the figure to excuse a smaller one – against print's own 0.9, where a figure *is* apparatus inside a column of prose. Bounded 0.6–1.6, more tightly than the scales, because the figure's width is this number times its viewBox measured in labels: a large value caps the drawing at the column, and a slide's own type then comes down to meet it.

**The greys have a hue, and it was nobody's decision until it was a key.** In the four light themes the `A` cycle moves `--emph` and nothing else: `--ink` is fixed at chroma 0.01 on hue 260, `--paper` and `--rule` at chroma 0. Every tinted surface – a card at 5% of the ink, a dock and an overlay card at 4% – is mixed out of `--ink`, so a card under a warm accent is a cool grey under a warm word, and `light-blue` is the one theme where the two agree. `style: {neutrals: …}` takes `neutral` (the default, and today's rendering byte for byte), `tinted` (the greys take the accent's own hue and the fills mix from `--emph`), `warm` and `cool` (a fixed hue, held off the two terminal themes, where a single phosphor tone is the whole design). `print-neutrals` is the same four words for the two documents, and it exists because the two grounds are not the same ground: print's paper is `#fafaf7` and its accent `#8b2e00`, both warm, where the live paper is at chroma 0. Its default is a deferral rather than a value, so an unset key follows `neutrals` and writing one key alone answers for both – the shape `print-slide-numbers` carries, with the same trap, that a second reader supplying a fallback would silently make unset mean `neutral`.

**A title pair can be set either way up.** A cover carries a pair through `title:` and `subtitle:`, a divider and a closing slide through `Heading | Sub`, and until `style: {headline: …}` the first line was the large one with no way to say otherwise. `stacked` is the title over a quieter subtitle and the default; `eyebrow` sets the title small above a subtitle that carries the weight, which is the shape a lecture title takes when the first line names the field and the second asks the question. The words do not move and only their type does, which is what lets one key serve the cover, the dividers and the closing slide at once: `title:` stays the content key of whichever line is loud, because it is also the `<title>` element, the contents entry and what the search index reads, and telling authors to put the hook there would rename the browser tab to the hook and leave the lecture's own name nowhere. `style: {caps: …}` sets the small type round a title in capitals – the eyebrow, the presenter, the affiliation, and deliberately not the headline, a key that capitalises the loud line being a key that makes a talk shout. The tracking is not a second key: capitals at the tracking of lowercase read as one jammed word, which is a typographic rule rather than a preference, so the build tracks out any slot already in capitals, including one an author typed that way.

**And it can decide where the things that are not prose sit.** Three of them – a code block, a figure with its caption, a display formula – have been centred since 1.0.0, and centred is right when the block *is* the slide. It is wrong when the block is one step of an argument: a paragraph, then a formula, then a paragraph reads as three blocks on three axes. `style: {blocks: left}` starts all three where the sentence above them starts; `center` is the default and writing it changes nothing. A `::: draw` is outside the key, because a diagram fills the measure at every chunk width and has no space beside it to align in.

**Two of these keys change their mind from slide to slide, and only those two have a per-chunk form.** `{.blocks-left}`, `{.blocks-center}`, `{.wrap-none}` and `{.wrap-balance}` in a chunk's attribute tail override `blocks` and `wrap` for that chunk. Each is spelled as the key's own name and one of its values, so an author who knows the frontmatter can guess the class and the other way round, and both directions of both keys exist because under a deck-wide `wrap: none` the only way left to ask for balancing is to ask for it on the chunk. `headings`, `rules` and `labels` get no such class: they are decisions a deck makes once. Unlike `{.bare}` and `{.center}`, these four reach the printed document, because the keys they answer always have – where a formula sits relative to the paragraph that introduced it is the same question on paper as on the wall.

**Hyphenation is one of those decisions and not a property of the language.** `lang:` picks the dictionary – that is a fact about the lecture – but whether a word may break at the end of a line is a preference, and the two views disagree about it by default: the printed document hyphenates its prose and the projection never has, because a broken word reads badly across a room and a live view reflows under the reader's hands. `style: {hyphenate: …}` is the say over both halves: `print` is that default, `all` puts it into the live views too, `none` takes it out everywhere. The case for `all` is the one that forced it – a German compound at `.narrow`, where one word opens a hole in the measure that no rewriting closes. On the projection it is scoped to the slide, so a contents list, a search hit and the help sheet are never hyphenated.
...
- **Annotation box** over the frame while typed, a margin note beside the text column at rest (see §2 annotation slot).
...
- **Below the top level.** A `---` inside a `::: side` pane, a `::: cards` / `::: rows` block, an `::: overlay` card, a `::: dock` or under a `#` heading is a beat on the same counter – it cannot split the chunk (a block cannot straddle two segments), so the runtime hides everything after it inside its block until its beat. The counter runs in source order over the whole slide, top-level and nested mixed; inside an `::: overlay from N` the inner beats count from `N`. A nested beat **keeps its box**: the block stands at its final height from beat 0 and the words fade in where they were going to be, so a row that arrives line by line or a card row whose tallest card comes last does not move the slide. `::: expand` and `::: script` keep the horizontal rule – neither is on the projection. **`--- from N` pins a beat to an advance by number** instead of to its place in the order, which is what a slide needs when two things that should arrive together are written in two places – a stepped figure in one pane of a `::: side` and the prose about it in the other. A pinned beat rides one the slide already has rather than adding a position of its own; `from 0` is refused (that is the beat the slide opens on), and so is a `from` inside an `::: overlay from N` or a `::: dock from N`, which numbers its own markers already.
- **Room for what has not arrived.** A segment keeps its box before its beat, wherever it sits: the chunk stands at its final height from beat 0 and the words fade in where they were going to be. Up to this change only a *nested* beat did that, a top-level one closed up and the chunk grew per press, and `style: {reveal: hold}` bought the nested behaviour deck-wide. One rule now, and no key – a slide that changes height under the reader is a slide that jumps, and auto-fit measures a different height on every beat. Measured over seven lectures before the change, the set of chunks taller than the frame is the same either way, because the last beat shows every segment under both rules.
...
The DSL takes free placement from the second and connectivity from the first, and drops auto-layout. The opener is `::: draw [WxH] [autoplay N [cycle]]` – the grid positional, playback as keywords, and no braces: everywhere in the format a `{…}` tail holds sigil tokens only (`.word`, `#word`, and inside a draw body `@word` and `!word`), so the one block line that carries values has none.
...
| `default <kind> [@tag] {…} [options]` | The base styling and size for that kind, optionally only for elements carrying a tag. The options are exactly the ones that kind's own statement takes – `w`/`h` for a box, text or image, `r` for a dot, `pad` for a container or brace |
...
**Routing is the author's, with one bounded exception.** An edge is straight segments through the waypoints written for it; nothing steps around a box. `.elbow` is the exception: it turns once out and once in, the rail sits across the gap on whichever axis the two ends are further apart, it looks at nothing else in the figure, and the author has no option to move it. Halfway, except that the run *after* the rail – the only part of the route that says which box the arrow points into – is held to one label plus an arrowhead, and where halfway would leave less the rail slides toward the source until it is that long. Where the gap cannot pay for that and a head's length out as well, the rail stays halfway and the build reports it, because the fix is then a number on the boxes. The rail is measured between the two elements' facing sides rather than between their centres, so several children of one parent turn on the same line and the set reads as one bracket, which is what a tree is made of. Writing `.elbow` and `via` on one edge is an error rather than a preference the build guesses at.
...
Inside a label, `_sub` and `^sup` shift a character or a `{group}`, `*accent*` colours a run with the theme accent and `~muted~` greys it. This exists because a sentence with two colour changes otherwise needs one `text` element per run, chained with `right of` – unreadable as source, and re-sorted by hand every time a word changes.

**Four markers, and a backslash writes any of them literally.** `\_`, `\^`, `\*`, `\~` and `\\` each draw the bare character and do nothing else, which is how a label says `scan\_page` or `MAX\_RETRIES`; an `*` or `~` with no partner later in the label is a literal character on its own, and an escaped one is no partner. `_` and `^` have no such fallback available to them – they take the character that follows and there is nothing to be unmatched against – so before the escape existed every snake_case identifier was unwritable: `scan_page` drew as `scan`, a subscript `p` and `age`, silently, with a clean lint and a box measured to fit exactly the wrong reading. The escape covers all four markers rather than only the two that need it, because an escape with an exception in it is one more thing to remember. It is resolved where the label is split into spans, so it reaches neither the measured width nor the drawing: a box carrying one is no wider for it. A backslash before anything else is an ordinary backslash, and so is a trailing one – with the single exception of the two sequences the quoted string itself owns, `\n` (a line break) and `\"` (a quote that does not end the string).

**A grid cell is not square, so `w` and `h` are a poor way to say what shape something is.** They are counts of grid cells, and on a `150x52` grid a plot written `w 1.9 h 1.5` lands 285px by 78px – nothing on that line hints at it, and it is the one place where the coordinate system leaks into a decision the author is making about the *picture*. `aspect W:H` on a `bars` or a `plot` states the proportion the reader sees and lets the build work the other number out. It is the only dimension in the grammar expressed in page terms rather than grid terms, and it is expressed that way because the grid is exactly what obscures it. Giving `w`, `h` and `aspect` together is refused: two ways of stating one number is two ways of stating different ones. `same as <chart>` is the third way, for the row of comparable frames the shared-baseline advice asks for, and it is the one place `same as` is answered while the line is read rather than during layout - a chart's contents are positioned from its own dimensions at parse time, so a size arriving later would move the frame and leave the gridlines behind it.
...
**The two zooms a wheel or a trackpad reaches – the overview board and the focus card – scale with the distance the hand moved, and they zoom where the pointer is.** Both used to answer a wheel event with a fixed factor picked off the sign of `deltaY`, which made the zoom a function of how many events arrived. Measured on a macOS trackpad in Chrome, an event arrives every 8.4 ms carrying between 0.012 and 6 px, so twenty-two of them crossed the card's whole 1×–8× range in 183 ms, and the inertia tail macOS keeps sending after the fingers lift – `deltaY` around 0.2, the sign flipping as it dies – went on stepping the zoom after the gesture was over. The step is `exp(-deltaY · k)` with `k` at 0.01, about 208 px of finger travel for the card's full range, and the per-event delta is clamped at 12 px: no trackpad event is touched, while a mouse wheel's 100–120 px notch becomes 13% rather than 3.3×. Scaling by `exp()` composes, so several events can be coalesced into one frame without the result depending on how they were grouped. Anchoring is the second half: the point under the pointer stays under the pointer, `pan' = pan + (1 - r) · (Q - visibleCentre)` for the card and one step on `manualPan` for the board, with `r` the ratio actually achieved rather than the one requested – at the 8× ceiling a requested ratio would go on sliding the card under a cursor that is no longer zooming anything. `+` and `-` still zoom from the centre, because a keypress carries no pointer.
