...
The class vocabulary is a closed enumeration of 29 names, and every one of them occupies a **slot**: fill (`tone-1`…`tone-4`, `clear`, `paper`), ink (`accent`, `muted`), stroke pattern (`dashed`, `dotted`), stroke weight (`thick`, `bare`), corner (`round`, `sharp`), size (`small`, `large`), family (`mono`, `serif`, `hand`), fit (`fit`, `shrink`), text alignment (`left`, `right`). Two members of one slot on one element is a lint warning; a class in a slot a `default` already filled displaces it rather than stacking.
