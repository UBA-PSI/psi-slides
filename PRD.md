...
The bar for adding one is the same each time, and it is not expressiveness: **the hand-built version has to be the thing that cannot be maintained.** A table of six rows by three columns costs twenty-one hand-named boxes and a chain of relative placements that has to be re-aimed whenever a row is inserted, which is the edit a diff and a reviewer both handle worst. A swimlane's bands cannot be `container`s at all, because a container fits its members and bands holding different numbers of things then come out ragged at both ends. A flowchart, a tree and a sequence of messages need no statement of their own – they are boxes and edges arranged, and `figure-design.md` shows each of them in a dozen lines.
