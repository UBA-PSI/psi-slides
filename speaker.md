...
- **The prompter** (`--souffleuse` only, `PLAN-souffleuse.md`): a live prompter that listens to the room and whispers back at most twelve words. Five pieces of chrome, all of them this window's:
  ...
  **The interim line** `#souffleuse-heard` sits under the strip and shows the last eight or so words the ear has provisionally heard. Off by default: reassuring while rehearsing, one moving line too many in front of a room.
