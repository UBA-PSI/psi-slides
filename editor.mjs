...
function dgeStartMove(ev, pt0) {
  ...
  const move = (e) => {
    ...
    const res = dgeMoveSelection(ctx, dx, dy, { free: e.ctrlKey || e.metaKey });
    last = { edits: res.plan ? res.plan.edits : [], refusals: res.refusal ? [res.refusal] : [] };
    DGE.source = res.next;
    dgeRecompile();
...
function dgeRenderBeats() {
  ...
  host.hidden = false;
  host.appendChild(dgeEl('span', { class: 'dge-cap', text: 'beat' }));
...
function dgeStoreKey(fig) {
  return DGE_STORE + (fig.chunk || fig.prefix);
}
...
function dgeApplyToPage(fig, body) {
  ...
  if (live && payload) {
    try {
      live.data = JSON.parse(payload.textContent);
      live.svg = next;
      live.cache = {};
      live.cur = null;
      next.psiDiagram = live;
      window.dgStep(live, live.step, true);
    } catch (e) { /* a figure with no steps has no payload */ }
  }
  return true;
}
...
function dgeRenderAll() {
  ...
  if (copy) {
    ...
    copy.title = live
      ? 'patch this block back into source.md through the --watch socket'
      : DGE.fileHandle ? 'patch this block back into the file you opened'
