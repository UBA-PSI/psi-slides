...
- know the typical **structure of scientific texts**,
- be able to **develop a suitable structure** for a topic,
- be able to **create an outline** from source material.

## principle: Organization is thinking {.standard #why-outlines}

Good organization helps writers **think more clearly** and present their arguments more effectively.

Starting with a question and growing an outline from it lets new ideas emerge – organically, rather than being forced in after the fact.

> note: This is the central claim of the session. Everything else is in service of it. Don't rush.

# What to include {#what-to-include}

## question: Am I done yet? {.narrow #am-i-done}

What actually belongs in a scientific text?

> note: Pause. Let the room answer first. Collect a few answers on the board before revealing the next chunk.

## free: Dimensions to cover for each section {.wide #dimensions}

For every section or referenced source, check whether you have addressed:

- **Key terms and concepts** – define anything critical to understanding.
...
::: margin
A minimal version of this list also works as a per-paragraph check during revision.
:::

# Expanding a topic {#expanding}

## question: Am I really done yet? {.narrow #am-i-really-done}
...
Explore the same concept in a **different domain or field**. New perspectives from an unfamiliar field often transfer back and enrich the original analysis.

## free: Intra-domain exploration {.standard #intra-domain}

Stay inside the domain but look at **neighboring aspects**. Connections, contrasts, and parallels within the same field often surface ideas that are not immediately obvious.

## free: Lenses and frameworks {.standard #lenses}

Apply a **theoretical lens** – economic, social, psychological, philosophical. Each lens foregrounds different aspects of the same topic.

## free: Stakeholder analysis {.standard #stakeholders}

Identify who is **involved in or affected by** the topic. Different stakeholders reveal different implications and tensions.

## free: Counterfactual thinking {.standard #counterfactual}

Explore **what-if scenarios** and alternative outcomes. Counterfactuals expose hidden assumptions, gaps, and weak links in an argument.

## free: Critical questioning {.standard #critical-questioning}
...
## free: Sort it out – the process {.wide #sort-it-out}
...
Pick a structure that matches the **shape of the content**:

- **Chronological** – order of events or steps. Narratives, procedures, histories.
...
- **Pro–con / advantages–disadvantages** – balanced view of an issue or option.

> note: Worth asking: for their own topic, which two schemes fit, and which one wins? The answer is rarely obvious.

## free: Organizing the main part {.wide #organize-main-part}

Zooming in on the body of the paper:
...
## figure: An outline – ML in medical diagnoses {.full #ml-outline}

```
I.   Introduction
     A. Background on machine learning in medicine
     B. Problem statement
     C. Thesis statement

II.  Problem: Inaccuracies in traditional diagnoses
     A. Misdiagnoses and their consequences
     B. Limitations of human experts

III. Solution: Machine learning algorithms
     A. Overview of ML algorithms in diagnoses
     B. Advantages of machine learning algorithms
        1. Increased accuracy
        2. Efficiency and time-saving
        3. Scalability
     C. Examples of successful implementations

IV.  Challenges and limitations of ML algorithms
...
V.   Conclusion
     ...
     B. Implications for the future of diagnoses
```

A **problem–solution** outline for a paper on machine learning in medical diagnosis.

## example: The same outline, with meta-comments {.full #ml-outline-meta}

Adding meta-commentary (in **bold**) turns the outline into a working document – a thinking tool, not just a table of contents.

```
I.   Introduction
     – Purpose: motivate the reader.
     A. Background on machine learning in medicine
     B. Problem statement
     C. Thesis statement
...
III. Solution: Machine learning algorithms
     – Relevance: the title promises solutions; here is where that promise is kept.
     – Purpose: demonstrate technical depth.
     A. Overview of ML algorithms in diagnoses
        – Importance: lets the reader follow the next sections.
        – Scope: include decision trees; exclude artificial-ant-tree hyperplanes (too complex).
...
## free: Practicalities {.wide #practicalities}

Small habits that keep outlining productive:
...
## exercise: Checklist II – typical outline mistakes {.wide #checklist-mistakes}

Watch for these failure modes – all common, all fixable:

- **Lack of focus.** Too many unrelated ideas dilute the main purpose.
...
- **Neglected references.** Sources are not integrated into the outline – or not cited later in the paper.

> note: Finish by asking each student to point at the single mistake they are most prone to. Homework picks up from there.
