...
An attribute tail may also carry the non-width classes: `.bare` and `.center`
(audience-only), `.wrap-none` / `.wrap-balance` / `.blocks-left` /
...
`closing` chunk except the `style:` ones. **The vocabulary, what each one costs,
